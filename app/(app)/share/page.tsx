...
import { Clock, Copy, Trash2, Plus, Shield, Eye, Check, Loader2 } from 'lucide-react'
...
function CreateModal({ uid, open, onClose }: CreateModalProps) {
  ...
  async function generate() {
    ...
    try {
      ...
      const docRef = await createShareLink(uid, {
              ...
              {[
                ...
                <div key={key} className="card p-4 flex items-center gap-3">
                  ...
                  <button
                    ...
                    className={cn('w-12 h-6 rounded-full transition-all relative', (config as Record<string, unknown>)[key] ? 'bg-gold-500' : 'bg-vault-muted')}
                  >
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {EXPIRY_OPTIONS.map((opt) => (
                    <button
                      key={opt.label}
                      onClick={() => setConfig({ ...config, expiryHours: opt.hours })}
                      className={cn(
                        'py-2.5 rounded-xl font-bold text-sm transition-all',
                        config.expiryHours === opt.hours
            ...
            <div className="space-y-4">
              <div className="card p-4 border-green-500/20 bg-green-500/5 text-center">
                ...
                <p className="text-xs text-vault-text-muted mt-1">
                  Expires in {EXPIRY_OPTIONS.find((o) => o.hours === config.expiryHours)?.label}
                </p>
              </div>
              ...
              <button
                ...
                className={cn(
                  ...
                  copied ? 'bg-green-500 text-white' : 'bg-gold-gradient text-charcoal-300 hover:shadow-gold-glow'
                )}
              >
...
export default function SharePage() {
  ...
  return (
    <div className="min-h-dvh bg-vault-bg">
      ...
      <div className="px-5 mt-2 pb-28">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-xs text-gold-500 font-semibold">
            {links.filter((l) => !isExpired(l) && l.status === 'active').length} active
          </span>
        </div>
        ...
        {loading && (
          <div className="flex flex-col items-center py-12 gap-3">
            <Loader2 size={28} className="text-gold-500 animate-spin" />
            <p className="text-sm text-vault-text-muted">Loading links…</p>
          </div>
        )}
                    ...
                    <div className="flex items-center gap-3 mt-2.5 flex-wrap">
                      {!expired ? (
                        <div className="flex items-center gap-1 text-xs font-semibold text-gold-500">
                          <Clock size={12} />
                          {formatCountdown(link.expiresAt.toDate())}
                        </div>
                      ) : (
      ...
      {user && (
        <CreateModal uid={user.uid} open={showCreate} onClose={() => setShowCreate(false)} />
      )}
    </div>
  )
}
